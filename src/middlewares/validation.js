...
const validateSisterUpdate = [
  body('birth_name').optional().notEmpty().withMessage('birth_name cannot be empty'),
  body('date_of_birth')
    .optional()
    .isISO8601()
    .withMessage('date_of_birth must be a valid date'),
  body('place_of_birth').optional().notEmpty().withMessage('place_of_birth cannot be empty'),
  optionalEmail('email'),
  optionalPhone('phone'),
  optionalPhone('emergency_contact_phone'),
  optionalDate('baptism_date'),
...
const validateCommunityAssignmentCreate = [
  body('sister_id').isInt({ min: 1 }).withMessage('sister_id must be a positive integer'),
  body('community_id').isInt({ min: 1 }).withMessage('community_id must be a positive integer'),
  body('role')
    .notEmpty()
    .withMessage('role is required')
...
const validateVocationJourneyCreate = [
  ...
  body('stage')
    ...
    .isIn(['inquiry', 'postulant', 'aspirant', 'novice', 'temporary_vows', 'perpetual_vows', 'left'])
...
const validateMissionCreate = [
  ...
  body('field')
    ...
    .isIn(['education', 'pastoral', 'publishing', 'media', 'healthcare', 'social'])
...
const validateEvaluationCreate = [
  body('sister_id').isInt({ min: 1 }).withMessage('sister_id must be a positive integer'),
  body('evaluation_period').notEmpty().withMessage('evaluation_period is required'),
  ['spiritual_life_score', 'community_life_score', 'mission_score', 'personality_score', 'obedience_score'].map((field) =>
    body(field)
      .optional({ checkFalsy: true })
      .isInt({ min: 0, max: 10 })
