...
const ensurePermission = (req, res, roles, { allowDelegate = false, permission = '' } = {}) => {
  if (!req.user) {
  ...
  if (allowDelegate && permission && hasDelegatedPermission(req.user, permission)) {
    return true;
  }
...
const getHealthRecordBySister = async (req, res) => {
  try {
    if (!ensurePermission(req, res, extendedViewRoles, { allowDelegate: true, permission: 'HEALTH_READ' })) {
      return;
    }
    ...
    return res.status(200).json({
      sister: { id: sister.id, code: sister.code, religious_name: sister.religious_name },
      records
    });
  } catch (error) {
...
const addHealthRecord = async (req, res) => {
  try {
    if (!ensurePermission(req, res, privilegedRoles, { allowDelegate: true, permission: 'HEALTH_WRITE' })) {
      return;
    }

    const { sister_id: sisterId, general_health: generalHealth } = req.body;
    if (!sisterId || !generalHealth) {
      return res.status(400).json({ message: 'sister_id and general_health are required' });
    }
...
const getGeneralHealthStatus = async (req, res) => {
  try {
    if (!ensurePermission(req, res, generalHealthRoles, { allowDelegate: true, permission: 'HEALTH_SUMMARY' })) {
      return;
    }
    ...
    return res.status(500).json({ message: 'Failed to fetch general health status' });
  }
};
