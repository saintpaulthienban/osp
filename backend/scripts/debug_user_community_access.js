  ...
  try {
    ...
    if (userCommunities.length === 0) {
      ...
      userCommunities.forEach((c, index) => {
        console.log(
          `   ${index + 1}. ${c.name} (ID: ${c.id}, Code: ${c.code})${
    ...
    if (user.is_admin || user.is_super_admin) {
      console.log(
        `   ✅ ADMIN: Can see ALL communities (${allCommunities.length} communities)`
      );
      console.log(
        `   Community IDs: [${allCommunities.map((c) => c.id).join(", ")}]`
      );
    } else if (userCommunities.length > 0) {
      console.log(
        `   👤 NON-ADMIN: Can only see assigned communities (${userCommunities.length} communities)`
      );
      console.log(
        `   Community IDs: [${userCommunities.map((c) => c.id).join(", ")}]`
      );
    } else {
    ...
    if (accessibleCommunityIds.length === 0) {
      ...
      Object.keys(byCommunity).forEach((communityName) => {
        ...
        byCommunity[communityName].forEach((s, index) => {
          console.log(
            `      ${index + 1}. ${s.saint_name || "N/A"} (${s.code})`
          );
        });
      });
    }
    ...
    if (orphanSisters.length > 0) {
      ...
      orphanSisters.forEach((s, index) => {
        console.log(
          `      ${index + 1}. ${s.saint_name || "N/A"} (${
            s.code
          }) - current_community_id: ${s.current_community_id || "NULL"}`
        );
      });
      console.log(
