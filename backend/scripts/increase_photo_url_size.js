...
async function increasePhotoUrlSize() {
  ...
  try {
    ...
    // Read migration file
    const migrationPath = path.join(__dirname, "../db/migrations/increase_photo_url_size.sql");
    const migrationSQL = fs.readFileSync(migrationPath, "utf8");
    
    // Execute migration
    await connection.query(migrationSQL);
    
    console.log("✅ Successfully increased photo_url column size to VARCHAR(1000)");
