...
async function testSisterCommunityFilter() {
  try {
    ...
    if (communities.length < 2) {
      console.log("⚠️  Need at least 2 communities. Creating test communities...");
      await db.execute(
    ...
    if (existingSisters.length >= 2) {
      ...
      console.log(`✓ Using existing sister 1: ${sister1.saint_name} (ID: ${sister1.id})`);
      console.log(`✓ Using existing sister 2: ${sister2.saint_name} (ID: ${sister2.id})`);
    } else {
      ...
      console.log(`✓ Created sister 1: ${sister1.saint_name} (ID: ${sister1.id})`);
      console.log(`✓ Created sister 2: ${sister2.saint_name} (ID: ${sister2.id})`);
    }
    ...
    console.log(`✓ Granted permissions to ${testUsername} for ${community1.name} only`);
    ...
    filteredSisters.forEach((sister, index) => {
      console.log(`  ${index + 1}. ${sister.saint_name} (${sister.code}) - ${sister.community_name}`);
    });
    ...
    if (shouldSee) {
      ...
      console.log(`  \u2192 ${shouldSee.saint_name} from ${shouldSee.community_name}`);
    } else {
      console.log(`\u2717 FAIL: User CANNOT see sister from assigned community`);
      console.log(`  \u2192 Should see ${sister1.saint_name} from ${community1.name}`);
      passed = false;
    }
    
    if (!shouldNotSee) {
      console.log(`\u2713 PASS: User CANNOT see sister from non-assigned community`);
      console.log(`  \u2192 ${sister2.saint_name} from ${community2.name} is correctly hidden`);
    } else {
      console.log(`\u2717 FAIL: User CAN see sister from non-assigned community`);
      console.log(`  \u2192 Should NOT see ${sister2.saint_name} from ${community2.name}`);
      passed = false;
    }
    ...
    const [result1] = await db.execute(getByIdQuery, [sister1.id, ...communityIds]);
    if (result1.length > 0) {
      console.log(`\u2713 PASS: User CAN access sister ${sister1.saint_name} by ID`);
    } else {
      console.log(`\u2717 FAIL: User CANNOT access sister ${sister1.saint_name} by ID`);
      passed = false;
    }
    
    // Try to get sister2 (should fail)
    const [result2] = await db.execute(getByIdQuery, [sister2.id, ...communityIds]);
    if (result2.length === 0) {
      console.log(`\u2713 PASS: User CANNOT access sister ${sister2.saint_name} by ID (correctly blocked)`);
    } else {
      console.log(`\u2717 FAIL: User CAN access sister ${sister2.saint_name} by ID (should be blocked)`);
      passed = false;
    }
    ...
    console.log("Cleanup: Run this script with --cleanup flag to remove test data");
